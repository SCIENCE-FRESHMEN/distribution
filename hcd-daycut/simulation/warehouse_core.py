...
class WarehouseCore:
    ...
    def __init__(self, num_aisles: int = 5, num_production_lines: int = 3,
        ...
        self.lr_balance_weight = float(cfg.get("lr_balance_weight", lr_balance_weight))
        self.makespan_weight = float(cfg.get("makespan_weight", 0.3))
        self.balance_weight = float(cfg.get("balance_weight", 0.001))
        self.production_line_avg_time_weight = float(cfg.get("production_line_avg_time_weight", 0.5))
        ...
        if not WarehouseCore._inbound_records_loaded:
            try:
                inbound_config = InboundConfigBuilder.load_json("simulation/data/inbound_task_config.json")
        ...
        self.crane_available_times: Dict[tuple, float] = {}  # {(production_line, aisle, side): time}
        # 事件驱动仿真相关
    ...
    def allocate_inbound_aisle(self, task_or_stub, current_time: float) -> Event:
        ...
        valid_production_lines = []  # 存储SKU可服务的所有产线
        ...
        for sku_entry in skus:
            if isinstance(sku_entry, dict):
                ...
                sid = None
            if sid is not None:
                pl_value = self.sku_to_production_line.get(sid)
                # sku_to_production_line可能返回列表
                if isinstance(pl_value, list):
                    valid_production_lines = [int(p) for p in pl_value]
                    production_line = valid_production_lines[0] if valid_production_lines else None
                else:
                    production_line = int(pl_value) if pl_value else None
                    valid_production_lines = [production_line] if production_line else []
                break
        # 巷道分配
        assigned_aisle = None
        ...
        if assigned_aisle is None:
            if valid_production_lines:
                # 找到可以服务任意一个有效产线的巷道
                valid_aisles = [aisle for aisle in self.aisles 
                               if any(pl in self.aisle_production_line_mapping[aisle] for pl in valid_production_lines)]
            else:
                valid_aisles = list(self.aisles)
            assigned_aisle = random.choice(valid_aisles) if valid_aisles else random.choice(self.aisles)
        # 生成任务ID（含入库线标记）
