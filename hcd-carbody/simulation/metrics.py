...
class MetricsCalculator:
    ...
    def __init__(self, aisles: List[int], sku_types: List[str] = None,
                 left_aisles: List[int] = None, right_aisles: List[int] = None,
                 lr_balance_weight: float = 00):
        """
        Args:
            aisles: 巷道列表
            sku_types: SKU类型列表（可为空，运行时从库存统计）
    ...
    def calculate_distribution_balance(self, current_inventory: Dict[int, Dict[str, int]]) -> float:
        ...
        sku_qty = []
        sku_set = set(self.sku_types)
        for aisle in self.aisles:
            sku_set.update(current_inventory.get(aisle, {}).keys())
        for sku in sku_set:
            quantities = [current_inventory.get(aisle, {}).get(sku, 0) for aisle in self.aisles]
            ...
            # 2. 
            left_qty = sum(current_inventory.get(aisle, {}).get(sku, 0) for aisle in self.left_aisles)
            right_qty = sum(current_inventory.get(aisle, {}).get(sku, 0) for aisle in self.right_aisles)
