...
class OptimizationScheduler:
    ...
    def solve(self, inbound_tasks: List[TaskData], outbound_tasks: List[TaskData], 
        ...
        # Hard-priority branch: empty-skid outbound requests selected by API should run first
        # when aisle is idle and unblocked in this cycle.
        forced_solution, outbound_tasks = self._extract_forced_high_priority_outbound(
            outbound_tasks, running_tasks, current_time
        )
        ...
        if forced_solution:
            for aisle, forced_tasks in forced_solution.items():
                ...
                # Remove duplicates if the same task appears in sampled solution.
    ...
    def _compute_feasible_assignments(self, outbound_tasks: List[TaskData], 
        ...
        for task in outbound_tasks:
            ...
            if match_mode == "features":
                ...
                # Single-SKU outbound in car body warehouse.
                sku = sku_ids[0]
                positions_by_aisle = {}
    ...
    def _sample_single_solution(self, all_tasks: List[TaskData],
        ...
        while inbound_idx < len(other_inbound_tasks_list) or outbound_idx < len(other_outbound_tasks_list):
            inbound_has = inbound_idx < len(other_inbound_tasks_list)
            outbound_has = outbound_idx < len(other_outbound_tasks_list)
    ...
    def _evaluate_and_select_best(self, solutions: Dict[str, Tuple], heuristic_solution: Dict[int, List[TaskData]], heuristic_future_ready: Dict[int, int] = None) -> Dict[int, List[TaskData]]:
        ...
        for idx, max_tasks in enumerate(max_tasks_values):
            base_w = 1.0 / max_tasks if max_tasks > 0 else 0.01  # 避免除以0，空方案权重很小但不为0
        ...
        for idx_eval, (solution, max_tasks) in enumerate(selected_solutions, start=1):
            # 计算基础得分
    ...
    #    heuristic_score = heuristic_base_score + heuristic_line_progress_balance + heuristic_empty_penalty
    #    self._log_score_breakdown(
    #        tag="heuristic",
    #        base_score=heuristic_base_score,
    #        score_details=heuristic_details,
    #        line_progress_balance_score=heuristic_line_progress_balance,
    #        empty_penalty=heuristic_empty_penalty,
    ...
    def _log_score_breakdown(
        ...
        """Log optimizer scoring decomposition."""
        d = score_details or {}
        wc = self.warehouse_core

        makespan = float(d.get("makespan", 0.0) or 0.0)
        makespan_w = float(getattr(wc, "makespan_weight", 0.0) or 0.0)
        makespan_part = float(d.get("makespan_score", makespan * makespan_w) or 0.0)

        balance_change = float(d.get("balance_change", 0.0) or 0.0)
        balance_w = float(getattr(wc, "balance_weight", 0.0) or 0.0)
        balance_raw = max(0.0, -balance_change) * 1000.0
        balance_part = float(d.get("balance_penalty", balance_raw * balance_w) or 0.0)

        avg_pl_time = float(d.get("avg_production_line_time", 0.0) or 0.0)
        pl_avg_w = float(getattr(wc, "production_line_avg_time_weight", 0.0) or 0.0)
        pl_avg_part = float(d.get("production_line_avg_score", avg_pl_time * pl_avg_w) or 0.0)

        pl_bal_w = float(getattr(wc, "production_line_balance_weight", 0.0) or 0.0)
        pl_bal_part = float(d.get("production_line_balance_penalty", 0.0) or 0.0)
        pl_bal_raw = (pl_bal_part / pl_bal_w) if pl_bal_w > 0 else 0.0

        aisle_disp_w = float(getattr(wc, "aisle_dispersion_weight", 0.0) or 0.0)
        aisle_disp_part = float(d.get("aisle_dispersion_penalty", 0.0) or 0.0)
        aisle_disp_raw = (aisle_disp_part / aisle_disp_w) if aisle_disp_w > 0 else 0.0

        inbound_wait = float(d.get("inbound_wait_time", 0.0) or 0.0)
        inbound_wait_w = float(getattr(wc, "inbound_wait_weight", 0.0) or 0.0)
        inbound_wait_part = float(d.get("inbound_wait_penalty", inbound_wait * inbound_wait_w) or 0.0)

        print(
            "[score][{tag}] final={final:.4f} = base({base:.4f}) + line_progress({lp:.4f}) + empty_penalty({ep:.4f})".format(
                tag=tag, final=final_score, base=base_score, lp=line_progress_balance_score, ep=empty_penalty
            )
        )
        print(
            "[score][{tag}] base={base:.4f} = "
            "{mw:.4f}*{ms:.4f}(makespan:{mp:.4f}) + "
            "{bw:.4f}*{br:.4f}(balance:{bp:.4f}) + "
            "{paw:.4f}*{pat:.4f}(pl_avg:{pap:.4f}) + "
            "{pbw:.4f}*{pbr:.4f}(pl_balance:{pbp:.4f}) + "
            "{adw:.4f}*{adr:.4f}(aisle_disp:{adp:.4f}) + "
            "{iww:.4f}*{iw:.4f}(inbound_wait:{iwp:.4f})".format(
                tag=tag,
                base=base_score,
                mw=makespan_w,
                ms=makespan,
