...
def load_aisle_max_columns(path: str = "config/warehouse.json") -> Dict[int, int]:
    ...
    default_cols = int(cfg.get("num_columns", 0) or 0)
    result: Dict[int, int] = {}
    if isinstance(dims, dict):
        for k, v in dims.items():
            ...
            if isinstance(v, dict):
                cols = int(v.get("columns", default_cols) or default_cols or 0)
                if cols > 0:
                    result[aisle] = cols
    if not result and default_cols > 0:
        num_aisles = int(cfg.get("num_aisles", 0) or 0)
        for aisle in range(1, num_aisles + 1):
            result[aisle] = default_cols
    return result
...
class TimeEstimator:
    ...
    def __init__(self,
                 ...
                 dock_in_col: int = 1,
                 dock_out_col: int = 3,
                 dock_map_in: Optional[Dict[int, Any]] = None,
                 dock_map_out: Optional[Dict[int, Any]] = None,
        ...
        self.dock_map_in = self._normalize_dock_map(
            cfg_dock_map_in or dock_map_in or {1: 1, 2: 5, 3: 9},
            default_col=self.dock_in_col,
        )
        self.dock_map_out = self._normalize_dock_map(
            cfg_dock_map_out or dock_map_out or {1: 1, 2: 11, 3: 16},
            default_col=self.dock_out_col,
        )
    ...
    def _normalize_dock_map(raw_map: Dict[Any, Any], default_col: int) -> Dict[str, Dict[str, int]]:
        ...
        Supports:
        - legacy mapping: {"1": 3}
        - col/level mapping: {"1": {"col": 17, "level": 3}}
        """
        ...
        for k, v in raw_map.items():
            ...
            normalized[line] = {"col": col, "level": level}
        return normalized
    ...
    def _clamp_dock_col(self, col: int, aisle: Optional[int]) -> int:
        if aisle is None:
            return int(col)
        try:
            max_col = int(self.aisle_max_columns.get(int(aisle), 0))
        except Exception:
            max_col = 0
        if max_col <= 0:
            return int(col)
    ...
    def resolve_inbound_dock(self, line: Optional[Any], default_layer: int = 1, aisle: Optional[int] = None) -> Tuple[int, int]:
        ...
        info = self.dock_map_in.get(line_key) if line_key is not None else None
        if not info:
            return self._clamp_dock_col(self.dock_in_col, aisle), int(default_layer)
        col = int(info.get("col", self.dock_in_col))
    ...
    def resolve_outbound_dock(self, line: Optional[Any], default_layer: int = 1, aisle: Optional[int] = None) -> Tuple[int, int]:
        ...
        line_key = self._dock_key(line)
        info = self.dock_map_out.get(line_key) if line_key is not None else None
        if not info:
            return self._clamp_dock_col(self.dock_out_col, aisle), int(default_layer)
    ...
    def estimate_inbound_time(self,
        ...
        roadway = int(target_position[0].aisle) if target_position and hasattr(target_position[0], "aisle") else 1
        dual = len(skus) > 1
        ...
        if dual:
            ...
            if len(target_position) < 2:
                ...
                df_tmp = pd.DataFrame([{
                    ...
                    "raw_layer": in_line if in_line is not None else 0,
    ...
    def estimate_outbound_time(self,
        ...
        """
        # return 70
        roadway = int(source_position[0].aisle) if source_position and hasattr(source_position[0], "aisle") else 1
        df_tmp = pd.DataFrame([{
            "InstructionID": 9998,
            "OUTorIN": 1,  # 出库
            "QROrder": 1,
    ...
    def _build_sequence(
        ...
        for roadway in df2["GL_Roadway"].unique():
            if start_pos_dict and not isinstance(start_pos_dict, dict) and hasattr(start_pos_dict, 'column') and hasattr(start_pos_dict, 'level'):
                ...
                if roadway in dock_map_in:
                    dock_in_info = dock_map_in.get(roadway, {}) or {}
                    start_pos_per_roadway[roadway] = (
                        self._clamp_dock_col(int(dock_in_info.get("col", dock_in_col)), roadway),
                        int(dock_in_info.get("level", default_dock_layer)),
                    )
                elif roadway in dock_map_out:
                    dock_out_info = dock_map_out.get(roadway, {}) or {}
                    start_pos_per_roadway[roadway] = (
                        self._clamp_dock_col(int(dock_out_info.get("col", dock_out_col)), roadway),
                        int(dock_out_info.get("level", default_dock_layer)),
                    )
                else:
        ...
        for roadway, g in df2.groupby("GL_Roadway", sort=True):
            ...
            for _, row in g.iterrows():
                ...
                raw_layer = None
                if task in [0, 1]:
                    raw_layer = row.get("raw_layer", default_dock_layer)
                raw_layer_key = self._dock_key(raw_layer)
                raw_layer_num = self._extract_level(raw_layer)

                if task == 0:  # 入库
                    if raw_layer_key is not None and raw_layer_key in dock_map_in:
                        dock_info = dock_map_in.get(raw_layer_key, {}) or {}
                        dock_col = self._clamp_dock_col(int(dock_info.get("col", dock_in_col)), roadway)
                        dock_layer = int(dock_info.get("level", raw_layer_num if raw_layer_num is not None else default_dock_layer))
                    else:
                        dock_col = self._clamp_dock_col(dock_in_col, roadway)
                        dock_layer = int(raw_layer_num) if raw_layer_num is not None else default_dock_layer
                elif task == 1:  # 出库
                    if raw_layer_key is not None and raw_layer_key in dock_map_out:
                        dock_info = dock_map_out.get(raw_layer_key, {}) or {}
                        dock_col = self._clamp_dock_col(int(dock_info.get("col", dock_out_col)), roadway)
                        dock_layer = int(dock_info.get("level", raw_layer_num if raw_layer_num is not None else default_dock_layer))
                    else:
                        dock_col = self._clamp_dock_col(dock_out_col, roadway)
                        dock_layer = int(raw_layer_num) if raw_layer_num is not None else default_dock_layer
                else:
                    dock_col = self._clamp_dock_col(dock_in_col, roadway)
